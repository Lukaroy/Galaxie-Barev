...
export async function POST(request: Request) {
  try {
    ...
    const sanitize = (s = '') => s.toLowerCase().replace(/[^a-z0-9_.-]/g, '_').slice(0, 32)
    const baseUserName = username ? sanitize(username) : sanitize(email.split('@')[0]) || `user_${uid.slice(0, 8)}`
    ...
    if (emailConflict && emailConflict.id !== uid) {
